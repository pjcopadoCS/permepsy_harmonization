from harmonization.common.cbq  import re_calcule as re
...
def replace_values(df,dataset,var):
    print("replacing values of ",var," in ",dataset)
    df.replace(to_replace={'A':1,'B':2,'C':3},inplace=True)    
...
def harmonize(df,dataset,var):
    ...
    # Add new columns (just the aggreted ones) to the dataframe in its proper position
    for indx,it in enumerate(new_items):
        if it not in df.columns:
            if 'pre_' in it:
                df.insert(df.columns.get_loc('pre_CBQ_30_0')+indx+1,it,0)
            else:
                df.insert(df.columns.get_loc('post_CBQ_30_0')+indx+1,it,0)
            df[it]=df[it].astype('int')
                
    dfp = df[df['database_0']==dataset][items]
    
    replace_values(dfp,dataset,var)    
    ...
    for i in ER:
        dfp['pre_CBQ_ER'+"_0"] += dfp['pre_CBQ_'+str(i)+"_0"]
        dfp['post_CBQ_ER'+"_0"] += dfp['post_CBQ_'+str(i)+"_0"]    
