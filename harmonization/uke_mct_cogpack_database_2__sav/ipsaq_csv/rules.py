...
def recover(df,dataset,var,items):
    ...
    df = reduce(df,dataset,var,total_items)
    
    total_items = [item for item in total_items if item.endswith('_0')]
...
def reduce(df,dataset,var,items):
    ...
    items_n = [item+'_0' for item in items_n]
    df = df[items_n]
...
def calcule(preffix,df,dataset,var,N):
    ...
    print("calculating aggragetes",dataset,preffix,var)
    ...
    items_agg = [item+'_0' for item in items_agg]
    return df,items_agg  
...
def harmonize(df,dataset,var):
    ...
    try:
        dfp,it = calcule('pre',dfp,dataset,var,N)
        dfp,it = calcule('post',dfp,dataset,var,N)
  
    except Exception as e:
        print("Error in harmonization: ",e)
	
    print("Final Dimensions:",dfp.shape)
