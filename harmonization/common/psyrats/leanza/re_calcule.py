...
def calcule(preffix,df,dataset,var,items,n_items):
    ...
    for indx,row in df.iterrows():
        ...
        try:
            df.loc[indx,f_item[0]]= row[o_item[:11]].sum(min_count=len(o_item[:11]))
        except Exception as e:
            df.loc[indx,f_item[0]] = pd.NA

        try:
            df.loc[indx,f_item[1]]= row[o_item[11:]].sum(min_count=len(o_item[11:]))
        except Exception as e:
